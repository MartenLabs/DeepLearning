...
dataset = apply_cutmix(dataset, max_people=4, iou_threshold=0.01, mode='all',  new_image_size=(128, 96), target_size=100000, padding=3)
...
class BackBone(tf.keras.layers.Layer):
    def __init__(self):
        ...
        self.L1dconv5 =layers.DepthwiseConv2D(kernel_size=3, padding='same', depth_multiplier=1, strides=1, kernel_regularizer=regularizers.l2(0.005))


        self.L3conv1 = Conv(out_channels=64, kernel_size=1, stride=2, padding='valid')  # 32
        self.L3conv2 = DepthwiseConv(out_channels=64, kernel_size=3, stride=1, padding='same')
        ...
        self.L3dconv5 = layers.DepthwiseConv2D(kernel_size=3, padding='same', depth_multiplier=1, strides=1, kernel_regularizer=regularizers.l2(0.005))


        self.L4conv1 = Conv(out_channels=64, kernel_size=1, stride=2, padding='valid')  # 16
        self.L4conv2 = DepthwiseConv(out_channels=64, kernel_size=3, stride=1, padding='same')
        ...
        self.L4dconv5 = layers.DepthwiseConv2D(kernel_size=3, padding='same', depth_multiplier=1, strides=1, kernel_regularizer=regularizers.l2(0.005))


        self.L5conv1 = Conv(out_channels=64, kernel_size=1, stride=2, padding='valid')  # 8
        self.L5conv2 = DepthwiseConv(out_channels=64, kernel_size=3, stride=1, padding='same')
        ...
        self.L5dconv5 = layers.DepthwiseConv2D(kernel_size=3, padding='same', depth_multiplier=1, strides=1, kernel_regularizer=regularizers.l2(0.005))
    ...
    def call(self, inputs):
        ...
        out1 = self.L3dconv5(L3_dconv4)



        L4_conv1 = self.L4conv1(L3_x2)
        ...
        out2 = self.L4dconv5(L4_dconv4)



        L5_conv1 = self.L5conv1(L4_x2)
        L5_conv2 = self.L5conv2(L5_conv1)
        L5_reduce_noise = self.L5_maxpool(L5_conv2)
        L5_dconv4 = self.L5dconv4(L5_reduce_noise)
        out3 = self.L5dconv5(L5_dconv4)
...
images, numbers, bboxes = dataset['images'], dataset['numbers'], dataset['bboxes']
print(len(numbers))
...
sample_size = min(1000, len(images_filtered))  # 샘플 사이즈 결정
indices = np.random.choice(len(images_filtered), sample_size, replace=False)

images_sampled = images_filtered[indices]
bboxes_sampled = bboxes_filtered[indices]
numbers_sampled = numbers_filtered[indices]
...
dataset = {
    ...
    'bboxes': bboxes_sampled,
    'numbers': numbers_sampled
}

# Load TFLite model
tflite_model_path = 'ObjectDetection/model_v10/tflite/model_v11_small_quant.tflite'
...
def draw_bboxes(image, true_boxes, pred_boxes, ap_score):
    ...
    for box in pred_boxes:
        ...
        ax.add_patch(rect)
        ax.text(x1, y1, f'Score: {tf.nn.sigmoid(score):.2f}', fontsize=10, color='b', bbox=dict(facecolor='white', edgecolor='none', pad=2.0))
...
def calculate_precision_recall(true_boxes, pred_boxes, iou_threshold=0.5):
    ...
    for i, pred_box in enumerate(pred_boxes):
        ...
        if best_iou >= iou_threshold and best_idx not in detected:
            ...
            false_positives[i] = 1
    false_negatives = len([box for box in true_boxes if not np.all(box == 0)]) - len(detected)
...
def compute_map(dataset, interpreter, iou_thresholds=[0.5]):
    aps = []
    image_ap_scores = []
    for idx, (img, true_boxes) in enumerate(zip(dataset['images'], dataset['bboxes'])):
        ...
        precision_list = []
        recall_list = []
        precision, recall = calculate_precision_recall(true_boxes, pred_boxes, 0.5)
        ...
        print(f'Image {idx + 1}/{len(dataset["images"])} - AP: {ap:.4f}, mAP so far: {current_map:.4f}')
...
def memory_usage_psutil():
    ...
    return mem_info.rss / (1024)  # KB 단위로 반환

# Load dataset
datasets = np.load('dataset/ObjectDetection.npz', allow_pickle=True)
images, numbers, bboxes = datasets['images'], datasets['numbers'], datasets['bboxes']

# Filter dataset for non-zero numbers
max_label_length = 4
labels = []
for num in numbers:
    cls = [1] * num if num != 0 else [0]
...
images_filtered = images_filtered[500:510]
bboxes_filtered = bboxes_filtered[500:510]
labels_filtered = labels_filtered[500:510]
numbers_filters = numbers_filters[500:510]
...
new_tflite_model_path = 'model_v11_small_quant.tflite'
interpreter = tf.lite.Interpreter(model_path=new_tflite_model_path)
interpreter.allocate_tensors()

# Measure memory after model load
memory_after = memory_usage_psutil()
memory_used_by_model = memory_after - memory_before
print(f'Memory used by the model: {memory_used_by_model:.2f} KB')

# Use the first 10 images from the dataset for inference
test_images = dataset['images'][:10]
resized_images = np.array([resize_and_pad_image(img, target_size) for img in test_images])
...
average_inference_time = run_inference(interpreter, resized_images)
print(f'Average Inference Time: {average_inference_time:.2f} ms')

# Get the model size
model_size = os.path.getsize(new_tflite_model_path) / 1024  # Size in KB
...
target_size = (256, 256)  # Set desired image size (width, height)

def resize_and_pad_image(image, target_size):
    # 이미지를 타겟 사이즈로 리사이즈
    resized_image = cv2.resize(image, target_size, interpolation=cv2.INTER_NEAREST)
    
    # 이미지가 그레이스케일인 경우 (2D 배열), 3채널 RGB 이미지로 변환
    if len(resized_image.shape) == 2:
        resized_image = cv2.cvtColor(resized_image, cv2.COLOR_GRAY2RGB)
    
    # 3채널 이미지인 경우 바로 3채널 이미지 배열 생성
    padded_image = np.zeros((target_size[1], target_size[0], 3), dtype=resized_image.dtype)
    padded_image[:resized_image.shape[0], :resized_image.shape[1], :] = resized_image
...
# Load the TFLite model
new_tflite_model_path = 'best_float16.tflite'
interpreter = tf.lite.Interpreter(model_path=new_tflite_model_path)
interpreter.allocate_tensors()
...
YoloV8n
45 epochs completed in 0.881 hours.
...
all    1682   3915     0.93    0.882    0.939     0.417
...
Memory used by the model: 264.00 KB
Average Inference Time: 13.62 ms
Model Size: 178.64 KB
mAP: 96.21

- YOLO V8n
INFO: Created TensorFlow Lite XNNPACK delegate for CPU.
Memory used by the model: 32940.00 KB
Average Inference Time: 144.99 ms
Model Size: 5966.25 KB
mAP: 93.90  
