...
class BackBone(tf.keras.layers.Layer):
    def __init__(self):
        ...
        self.L1conv1 = Conv(out_channels=8, kernel_size=3, stride=2, padding='same')   # 64
        self.L1conv2 = Conv(out_channels=16, kernel_size=3, stride=1, padding='same')
        self.L1conv3 = Conv(out_channels=32, kernel_size=3, stride=1, padding='same')
        
        self.L1cbam = CBAM()
        self.L1_maxpool = layers.MaxPool2D((1, 1))
        self.L1dconv4CBAM = Conv(16, kernel_size=1, stride=1, padding='same', kernel_regularizer=regularizers.l2(0.005))           
        self.L1dconv5CBAM = DepthwiseConv(out_channels=16, kernel_size=3, stride=1, padding='same') # 16       
    

        self.L3conv1 = Conv(out_channels=64, kernel_size=1, stride=2, padding='same')  # 32
        self.L3conv2 = DepthwiseConv(out_channels=64, kernel_size=3, stride=1, padding='same')
        self.L3_maxpool = layers.MaxPool2D((1, 1))
        self.L3cbam = CBAM()
        self.L3dconv4CBAM = Conv(32, kernel_size=1, stride=1, padding='same', kernel_regularizer=regularizers.l2(0.005))             
        self.L3dconv5CBAM = DepthwiseConv(out_channels=32, kernel_size=3, stride=1, padding='same')


        self.L4conv1 = Conv(out_channels=72, kernel_size=1, stride=2, padding='same')  # 16
        self.L4conv2 = DepthwiseConv(out_channels=72, kernel_size=3, stride=1, padding='same')
        self.L4_maxpool = layers.MaxPool2D((1, 1))
        self.L4cbam = CBAM()
        self.L4dconv4CBAM = Conv(36, kernel_size=1, stride=1, padding='same', kernel_regularizer=regularizers.l2(0.005))             
        self.L4dconv5CBAM = DepthwiseConv(out_channels=36, kernel_size=3, stride=1, padding='same')


        self.L5conv1 = Conv(out_channels=72, kernel_size=1, stride=2, padding='same')  # 8
        self.L5conv2 = DepthwiseConv(out_channels=72, kernel_size=3, stride=1, padding='same')
        self.L5_maxpool = layers.MaxPool2D((1, 1))
        self.L5cbam = CBAM()
        self.L5dconv4CBAM = Conv(36, kernel_size=1, stride=1, padding='same', kernel_regularizer=regularizers.l2(0.005))             
        self.L5dconv5CBAM = DepthwiseConv(out_channels=36, kernel_size=3, stride=1, padding='same')



    def call(self, inputs):
        L1_conv1 = self.L1conv1(inputs)
        L1_conv2 = self.L1conv2(L1_conv1)
        L1_conv3 = self.L1conv3(L1_conv2)
        L1_x1, L1_x2 = tf.split(L1_conv3, num_or_size_splits=2, axis=-1) # 16
        
        L1_reduce_noise = self.L1_maxpool(L1_x1)
        L1_dconv4CBAM = self.L1dconv4CBAM(L1_reduce_noise) 
        L1_dconv5CBAM = self.L1dconv5CBAM(L1_dconv4CBAM)
        L1_Add_X1 = layers.Add()([L1_dconv5CBAM, L1_reduce_noise])

        L1_concat = layers.Add()([L1_Add_X1, L1_x2])


        
        L3_conv1 = self.L3conv1(L1_concat)
        ...
        L3_dconv4CBAM = self.L3dconv4CBAM(L3_reduce_noise)
        L3_dconv5CBAM = self.L3dconv5CBAM(L3_dconv4CBAM)
        L3_Add_X1 = layers.Add()([L3_dconv5CBAM, L3_reduce_noise])

        out1 = layers.Add()([L3_Add_X1, L3_x2])
        ...
        L4_dconv4CBAM = self.L4dconv4CBAM(L4_reduce_noise)
        L4_dconv5CBAM = self.L4dconv5CBAM(L4_dconv4CBAM)
        L4_Add_X1 = layers.Add()([L4_dconv5CBAM, L4_reduce_noise])

        out2 = layers.Add()([L4_Add_X1, L4_x2])
        ...
        L5_dconv4CBAM = self.L5dconv4CBAM(L5_reduce_noise)
        L5_dconv5CBAM = self.L5dconv5CBAM(L5_dconv4CBAM)
        L5_Add_X1 = layers.Add()([L5_dconv5CBAM, L5_reduce_noise])


        out3 = layers.Add()([L5_Add_X1, L5_x2])
...
YoloV2
Used Flash : 329.45KiB
Used Ram : 280.30KiB
Total MAC: 110052238
Inference Time: 559.748ms

My Model
Used Flask : 176.83 KiB
Used Ram : 375.19KiB
Total MAC: 50308853 
Inference Time: 351.356 ms
Lowest AP Score: 0.6000
mAP: 0.9860
'''
```
