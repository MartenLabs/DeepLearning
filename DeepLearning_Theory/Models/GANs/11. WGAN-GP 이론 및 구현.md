...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, _) in enumerate(dataloader):
        ...
        # 실제 이미지를 적절한 텐서 타입으로 변환합니다.
        real_imgs = Variable(imgs.type(Tensor))

        # 판별자의 그래디언트를 0으로 초기화합니다.
        optimizer_D.zero_grad()

        # 생성자 입력으로 사용할 잡음을 샘플링합니다.
        z = Variable(Tensor(np.random.normal(0, 1, (imgs.shape[0], opt.latent_dim))))

        # 잡음으로부터 가짜 이미지 배치를 생성합니다.
        fake_imgs = generator(z)

        # 실제 이미지에 대한 판별자의 판별 결과를 계산합니다.
        real_validity = discriminator(real_imgs)
        # 가짜 이미지에 대한 판별자의 판별 결과를 계산합니다.
        ...
        # 손실에 대한 그래디언트를 역전파하고, 판별자의 파라미터를 업데이트합니다.
        d_loss.backward()
        optimizer_D.step()
        ...
        # n_critic 스텝마다 생성자를 한 번씩 훈련합니다.
        if i % opt.n_critic == 0:

            # 가짜 이미지 배치를 다시 생성합니다.
            fake_imgs = generator(z)
            # 생성자가 판별자를 얼마나 잘 속였는지에 대한 손실을 계산합니다.
            fake_validity = discriminator(fake_imgs)
            g_loss = -torch.mean(fake_validity)

            # 손실에 대한 그래디언트를 역전파하고, 생성자의 파라미터를 업데이트합니다.
            g_loss.backward()
            optimizer_G.step()

            # n_critic 스텝마다 배치 처리 수를 업데이트합니다.
            batches_done += opt.n_critic
            # 지정된 간격마다 로그를 출력하고 이미지를 저장합니다.
