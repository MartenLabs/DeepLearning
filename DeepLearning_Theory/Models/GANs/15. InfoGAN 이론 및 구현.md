...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.init_size = opt.img_size // 4  # 업샘플링 전의 초기 크기
        self.l1 = nn.Sequential(nn.Linear(input_dim, 128 * self.init_size ** 2))  # 첫 번째 선형 레이어
    ...
    def forward(self, noise, labels, code):
        ...
        out = out.view(out.shape[0], 128, self.init_size, self.init_size)  # 적절한 크기로 변경
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        ds_size = opt.img_size // 2 ** 4  # 다운샘플링된 이미지의 크기

        # 최종 출력 레이어: 진짜/가짜 판별, 클래스 레이블 예측, 연속적인 코드 추출
        self.adv_layer = nn.Sequential(nn.Linear(128 * ds_size ** 2, 1))  # 진짜/가짜 판별을 위한 레이어
        # 클래스 레이블 예측 레이어
...
static_code = Variable(FloatTensor(np.zeros((opt.n_classes ** 2, opt.code_dim))))  # 연속적인 코드(code) 초기화
```
