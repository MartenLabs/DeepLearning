...
- **객체 위치 학습**: `LabelEncoder`는 모델이 어떤 앵커 박스가 실제 객체를 포함하고 있는지, 그리고 그 객체의 정확한 위치가 어디인지 학습할 수 있도록 도와줍니다.
...
3. **손실 계산**: 모델의 예측과 인코딩된 라벨 사이의 차이(손실)를 계산합니다. 이 손실은 위치와 클래스 예측이 얼마나 정확한지를 나타냅니다.
4. **모델 최적화**: 손실을 최소화하기 위해 모델의 가중치를 조정합니다. 이 과정은 경사하강법(gradient descent)과 같은 최적화 알고리즘을 통해 수행됩니다.
5. **반복 학습**: 전체 데이터셋에 대해 이 과정을 여러 번 반복하여 모델의 성능을 향상시킵니다.

결과적으로, `LabelEncoder`는 모델이 각 앵커 박스에 대해 어떤 객체가 존재하는지(그리고 그 위치는 어디인지), 그리고 그 객체의 클래스는 무엇인지를 학습하는 데 필요한 정보를 제공합니다. 이는 객체 감지 모델의 핵심적인 부분으로, 모델의 성능과 정확도를 결정하는 데 중요한 역할을 합니다.
...
## Box Loss
$$L_{\text{box}}(y_{\text{true}}, y_{\text{pred}}) = \sum_i 
\begin{cases} 
\frac{1}{2}(y_{\text{true}}^i - y_{\text{pred}}^i)^2, & \text{if } |y_{\text{true}}^i - y_{\text{pred}}^i| \leq \delta \\
\delta|y_{\text{true}}^i - y_{\text{pred}}^i| - \frac{1}{2}\delta^2, & \text{otherwise}
\end{cases}$$


## Classification Loss
$$L_{\text{cls}}(y_{\text{true}}, y_{\text{pred}}) = \sum_i 
\begin{cases} 
-\alpha(1 - p_i)^\gamma \log(p_i), & \text{if } y_{\text{true}}^i = 1 \\
-(1 - \alpha)p_i^\gamma \log(1 - p_i), & \text{otherwise}
\end{cases}$$

## Total Loss
$$L_{\text{total}} = \lambda_{\text{cls}} L_{\text{cls}} + \lambda_{\text{box}} L_{\text{box}} + \lambda_{\text{CIoU}} L_{\text{CIoU}}$$
